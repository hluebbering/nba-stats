...
# NBA API
from nba_api.stats.endpoints import (playergamelog, boxscoreadvancedv2, leaguedashteamstats, scoreboardv2, commonplayerinfo, leaguegamefinder, boxscoretraditionalv2)
...
def get_opponent_stats(season='2024-25'):
    ...
    df = leaguedashteamstats.LeagueDashTeamStats(season=season, measure_type_detailed_defense='Defense', per_mode_detailed='PerGame', timeout=60).get_data_frames()[0]
...
def compute_true_shooting_percentage(df):
    ...
    df['TS_PCT'] = df.apply(lambda row: row['PTS'] / row['TS_DENOM'] if row['TS_DENOM'] != 0 else 0, axis=1)
...
def feature_engineering(player_df, adv_df, opp_df, team_map):
    ...
    for c in rolling_cols:
        df[f'{c}_AVG_LAST_5'] = (df.groupby('PLAYER_NAME')[c].transform(lambda x: x.shift(1).rolling(window=5, min_periods=1).mean()))

    for c in ['PTS', 'USG_PCT', 'MIN']:
        df[f'{c}_VOL_LAST_5'] = (df.groupby('PLAYER_NAME')[c].transform(lambda x: x.shift(1).rolling(window=5, min_periods=1).std()))
...
def get_team_defensive_stats(team_id, season='2024-25'):
    try:
        df = leaguedashteamstats.LeagueDashTeamStats(team_id_nullable=team_id, season=season, measure_type_detailed_defense='Defense', per_mode_detailed='PerGame', timeout=60).get_data_frames()[0]
...
# %%
def prepare_features_for_prediction(player_id, player_name, season='2024-25', feature_cols=DEFAULT_FEATURE_COLS, df_agg_position=None, df_agg_team_vs_opp=None):
    ...
    if df_agg_position is not None:
        ...
        if not pos:  # fallback
            pos = 'G'
        row_match = df_agg_position[(df_agg_position['OPPONENT_TEAM_ID'] == opp_team_id) & (df_agg_position['POSITION'] == pos)]
    ...
    if df_agg_team_vs_opp is not None:
        row_match = df_agg_team_vs_opp[(df_agg_team_vs_opp['TEAM_ID'] == p_team_id) & (df_agg_team_vs_opp['OPPONENT_TEAM_ID'] == opp_team_id)]
        if not row_match.empty:
...
# %%
def predict_upcoming_points(player_name, season='2024-25', feature_cols=DEFAULT_FEATURE_COLS, df_agg_position=None, df_agg_team_vs_opp=None):
    player_id = get_player_id(player_name)
    if not player_id:
        print(f"Invalid player: {player_name}")
        return None

    fv, latest_data = prepare_features_for_prediction(player_id, player_name, season, feature_cols=feature_cols, df_agg_position=df_agg_position, df_agg_team_vs_opp=df_agg_team_vs_opp)
    if fv is None:
        return None
...
for name in player_names:
    predict_upcoming_points(name, season, df_agg_position=df_agg_position, df_agg_team_vs_opp=df_agg_team_opp)
...
def scrape_espn_injuries(url="https://www.espn.com/nba/injuries"):
    ...
    for table_div in team_tables:
        ...
        for row in rows:
            ...
            # Store in a dict
            all_rows.append({"TEAM_NAME": team_name, "PLAYER_NAME": player_name, "POS": pos, "EST_RETURN": est_return, "STATUS": status, "COMMENT": comment})
